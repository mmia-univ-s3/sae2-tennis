...
def test_form_tournoi_delete(client, testapp):
    with testapp.app_context():
        response = login(client, "/competitions/tournoi/15/delete/")
        response = client.post('/competitions/tournoi/15/delete/',
            follow_redirects=True)
        ...
        response = login(client, "/competitions/tournoi/4/delete/")
        response = client.post('/competitions/tournoi/4/delete/',
            follow_redirects=True)
...
def test_form_tournoi_update(client, testapp):
    with testapp.app_context():
        response = login(client, "/competitions/tournoi/14/update/")
        response = client.post('/competitions/tournoi/14/update/',
        ...
        response = login(client, "/competitions/tournoi/3/update/")
        response = client.post('/competitions/tournoi/3/update/',
...
def test_form_participant_indiv_delete(client, testapp):
    with testapp.app_context():
        response = login(client, "/competitions/tournoi/12/3/delete/")
        response = client.post('/competitions/tournoi/12/3/delete/',
            follow_redirects=True)
...
def test_form_participant_indiv_add(client, testapp):
    with testapp.app_context():
        response = login(client, "/competitions/tournoi/13/add/")
        response = client.post('/competitions/tournoi/13/add/',
            follow_redirects=True, data={"joueur" : 1, "rang" : "1er"})

        assert response.status_code == 200
        assert "/competitions/tournoi/13/" in response.request.path
        assert b"Jean Claude" in response.data


def test_form_participant_equipe_delete(client, testapp):
    with testapp.app_context():
        response = login(client, "/competitions/tournoi/6/6/delete/")
        response = client.post('/competitions/tournoi/6/6/delete/',
            follow_redirects=True)

        assert response.status_code == 200
        assert "/competitions/tournoi/6/" in response.request.path
        assert b"Equipe : equipe 6" not in response.data


def test_form_participant_equipe_update(client, testapp):
    with testapp.app_context():
        response = login(client, "/competitions/tournoi/2/2/update/")
        response = client.post('/competitions/tournoi/2/2/update/',
            follow_redirects=True, data={"equipe" : 2, "rang" : "1er", "poule" : "A"})

        assert response.status_code == 200
        assert "/competitions/tournoi/2/" in response.request.path
        assert b"Classement : 1er" in response.data


def test_form_participant_equipe_add(client, testapp):
    with testapp.app_context():
        response = login(client, "/competitions/tournoi/2/add/")
        response = client.post('/competitions/tournoi/2/add/',
...
def test_form_affronter_delete(client, testapp):
    with testapp.app_context():
        response = login(client, "/competitions/tournoi/3/3/06-10-2025/delete/")
        response = client.post('/competitions/tournoi/3/3/06-10-2025/delete/',
            follow_redirects=True)

        assert response.status_code == 200
        assert "/competitions/tournoi/3/" in response.request.path
...
def test_form_affronter_update(client, testapp):
    with testapp.app_context():
        ...
        response = login(client, "/competitions/tournoi/5/5/08-10-2025/update/")
        response = client.post('/competitions/tournoi/5/5/08-10-2025/update/',
            follow_redirects=True, data={"adversaire" : "equipe 4", "date" : date.today(),
                                         "resultat": 'D', "score" : "10/12", "domicile" : "True"})

        assert response.status_code == 200
        assert "/competitions/tournoi/5/" in response.request.path
        ...
        response = login(client, "/competitions/tournoi/11/12/26-03-2023/update/")
        response = client.post('/competitions/tournoi/11/12/26-03-2023/update/',
        ...
        assert response.status_code == 200
        assert "/competitions/tournoi/11/12/26-03-2023/update/" in response.request.path
...
def test_form_affronter_add(client, testapp):
    with testapp.app_context():
        ...
        response = login(client, "/competitions/tournoi/3/3/add/")
        response = client.post('/competitions/tournoi/3/3/add/',
        ...
        response = login(client, "/competitions/tournoi/5/5/add/")
        response = client.post('/competitions/tournoi/5/5/add/',
            follow_redirects=True, data={"adversaire" : "adversaire1", "date" : date.today(),
                                         "resultat": 'D', "score" : "10/12", "domicile" : "True"})

        assert response.status_code == 200
        assert "/competitions/tournoi/5/5/add/" in response.request.path
