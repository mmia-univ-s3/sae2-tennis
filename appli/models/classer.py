...
class Classer(db.Model):
    ...
    id_comp:int = db.Column("idComp", db.Integer, db.ForeignKey("COMPETITION.idComp"), primary_key=True)
    competition = db.relationship("Competition", backref=db.backref("classer", lazy="dynamic", cascade="all, delete-orphan"))
    id_j:int = db.Column("idJ", db.Integer, db.ForeignKey("JOUEUR.idJ"), primary_key=True)
    joueur = db.relationship("Joueur", backref=db.backref("classer", lazy="dynamic", cascade="all, delete-orphan"))
